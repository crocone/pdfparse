 ...
 * => returns a function to be called for each item by the PdfParse.
 **/
Rule.makeItemProcessor = function (rules) {
  let currentAccumulator = null;
  function terminateAccumulator() {
    const terminatePreviousAcc = (currentAccumulator || {}).terminate;
  ...
  return function (item) {
    ...
    if (applyRulesOnNextItem)
      for (const r in rules) {
        let accumulator = rules[r].test(item);
        if (accumulator) {
...
Rule.addAccumulator("extractRegexpValues", function () {
  const matches = this.regexp.exec(this.currentItem.text);
